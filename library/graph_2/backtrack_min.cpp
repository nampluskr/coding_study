...
int res = 0;                    // optimal solution value (min / max)
...
void dfs_recur(const Edge& cur, int target, int query) {
    ...
    for (auto& next : adj[cur.to]) {
        if (!visited[next.to] && res < min(query, next.cost)) {
           ...
           dfs_recur(next, target, min(query, next.cost));
...
void dfs_stack(int start, int target) {
    ...
    S.push({ start, 0, INF, visited, path });
    ...
    while (!S.empty()) {
        ...
        for (auto& next : adj[cur.to])
            if (!visited[next.to] && res < min(query, next.cost)) {
                visited[next.to] = true; path.push_back(next.to);
                S.push({ next.to, next.cost, min(query, next.cost), visited, path });
                visited[next.to] = false; path.pop_back();
            }
    }
}
...
void bfs_queue(int start, int target) {
    ...
    queue<Edge2> Q;
    visited[start] = true; path.push_back(start);
    Q.push({ start, 0, INF, visited, path });
    ...
    while (!Q.empty()) {
        ...
        for (auto& next : adj[cur.to])
            if (!visited[next.to] && res < min(query, next.cost)) {
                visited[next.to] = true; path.push_back(next.to);
                Q.push({ next.to, next.cost, min(query, next.cost), visited, path });
                visited[next.to] = false; path.pop_back();
            }
    }
}
